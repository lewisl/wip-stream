...
| What if execution stops between remote and local changes? | Operation plans, receipts, and mutation-boundary phases |
...
3. In the same file, read `applyBidirectionalReconciliation()` for the shared publish-then-local-update path.
...
7. Read `execute()` last. It only turns one `git` subprocess into the raw `{ stdout, stderr, exitCode }` result consumed by the Git facade.
